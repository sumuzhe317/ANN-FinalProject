...
def train():
    ...
    dataroot = "/mnt/sda/2022-0526/home/scc/zty/code/ANN/finalproject/new_test_project/data/StanfordDogs/"
    ...
    for x, y in all_dataset:
        if y not in imgs:
            ...
            imgs[y].append(x)
        elif sum(len(imgs[key]) for key in imgs)==120:
            break
        else:
            continue
    ...
    os.makedirs("legacy/task4/figures/dog", exist_ok=True)
    os.makedirs("checkpoint/task4", exist_ok=True)
    os.makedirs("checkpoint/task4/dog", exist_ok=True)
    os.makedirs("checkpoint/task4/dog/gan", exist_ok=True)
    plt.savefig('legacy/task4/figures/dog/dog_true.jpg')
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(dataloader):
            ...
            # Save the Best Model
            if errG < loss_tep:
                torch.save(netG.state_dict(), 'checkpoint/task4/dog/gan/model.pt')
                loss_tep = errG
    ...
    plt.legend()
    plt.savefig('legacy/task4/figures/dog/loss.jpg')
    ...
    plt.legend()
    plt.savefig('legacy/task4/figures/dog/dx-dz.jpg')
    ...
    netG = Generator(0)
    netG.load_state_dict(torch.load('checkpoint/task4/dog/gan/model.pt', map_location=torch.device(device)))
    netG.eval()
    ...
    plt.savefig('legacy/task4/figures/dog/compare.jpg', bbox_inches='tight')
    ...
    source_gen_folder = 'data/StanfordDogs/Images/'
    target_gen_folder = 'data/StanfordDogs/genImages/'
